...
class HTTPProtocolSelectorTest {
	...
	fun testNextRequest(socket: ServerSocketChannel, version: HTTPVersion, latch: CountDownLatch) {
		while (true) {
			...
			connection.register(selector, SelectionKey.OP_READ or SelectionKey.OP_WRITE)
			val httpSelector = SocketHTTPProtocolSelector(selector, connection)
			val reading = SocketReadingByteBuffer(
				selector, connection,
				ByteBuffer.allocateDirect(8192), null
			)
			Thread.ofVirtual().start {
				httpSelector.setupServerReading(reading)
			}
//			try {
//				logger.info("Connection from [${connection.remoteAddress}]")
//				val request = httpSelector.nextRequest().getOrThrow()
//				logger.info("Request: $request")
//				httpSelector.sendResponse(
//					HTTPResponse(
//						request, 200,
//						data = ByteArrayChannel("Hello, ${version.tag}".toByteArray())
//					)
//				)
//				latch.await()
//				break
