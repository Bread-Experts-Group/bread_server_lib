...
class ISOBMFFParser(
	...
	override fun readBase(): ISOBMFFBox {
		val (name, data) = fqIn.read32ul().let { size32 ->
			...
			if (size64 > Int.MAX_VALUE) throw DecodingException("Size is too big [${size64.toULong()}]!")
			name to
	...
	fun containerBox(name: String) {
		this.addParser(name) { stream, box ->
			ISOBMFFContainerBox(
				box.tag,
