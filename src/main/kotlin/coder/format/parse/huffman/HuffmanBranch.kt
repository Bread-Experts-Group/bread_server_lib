package org.bread_experts_group.coder.format.parse.huffman
	...
	companion object {
		...
		fun HuffmanBranch<Char>.export(
			...
			if (value is HuffmanBranch<*>) (value as HuffmanBranch<Char>).export(
				...
				to.writeExtensibleULongV1((value as Char).code.toULong())
				to.writeExtensibleULongV1("1$stringValue${if (key) '1' else '0'}".toULong(2))
			}
		}
		...
		fun import(from: InputStream): HuffmanBranch<Char> {
			...
			while (from.available() > 0) {
				val value = Char(from.readExtensibleULongV1().toInt())
				val importing = from.readExtensibleULongV1().toString(2).substring(1)
