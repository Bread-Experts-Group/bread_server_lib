...
class WindowsSystemDeviceIODeviceFeature3(
	...
	override fun supported(): Boolean = nativeCreateFile3 != null && nativeCloseHandle != null &&
			nativeCreateDirectory2Wide != null && nativeGetFileInformationByHandleEx != null
	...
	companion object {
		...
		internal fun getDesiredAccess(
			...
			if (features.contains(WindowsIOReOpenFeatures.DELETE)) {
				data.add(WindowsIOReOpenFeatures.DELETE)
				desiredAccess = desiredAccess or 0x00010000
			}
			if (features.contains(WindowsIOReOpenFeatures.READ_CONTROL)) {
				data.add(WindowsIOReOpenFeatures.READ_CONTROL)
				desiredAccess = desiredAccess or 0x00020000
			}
			if (features.contains(WindowsIOReOpenFeatures.WRITE_DAC)) {
				data.add(WindowsIOReOpenFeatures.WRITE_DAC)
				desiredAccess = desiredAccess or 0x00040000
			}
			if (features.contains(WindowsIOReOpenFeatures.WRITE_OWNER)) {
				data.add(WindowsIOReOpenFeatures.WRITE_OWNER)
				desiredAccess = desiredAccess or 0x00080000
			}
			if (features.contains(WindowsIOReOpenFeatures.SYNCHRONIZE)) {
				data.add(WindowsIOReOpenFeatures.SYNCHRONIZE)
				desiredAccess = desiredAccess or 0x00100000
			}
			if (features.contains(WindowsIOReOpenFeatures.ACCESS_SYSTEM_SECURITY)) {
				data.add(WindowsIOReOpenFeatures.ACCESS_SYSTEM_SECURITY)
				desiredAccess = desiredAccess or 0x01000000
			}
			if (features.contains(WindowsIOReOpenFeatures.MAXIMUM_ALLOWED)) {
				data.add(WindowsIOReOpenFeatures.MAXIMUM_ALLOWED)
				desiredAccess = desiredAccess or 0x02000000
			}
		...
		internal fun getFlags(
			...
			return flags or (0x01000000 or 0x02000000) // FILE_FLAG_POSIX_SEMANTICS
		}
	}
	...
	override fun open(
		...
		val handle = Arena.ofConfined().use { ePA ->
			val extendedParameters = ePA.allocate(CREATEFILE3_EXTENDED_PARAMETERS)
			CREATEFILE3_EXTENDED_PARAMETERS_dwSize.set(extendedParameters, 0L, extendedParameters.byteSize().toInt())
			var attributes = CREATEFILE3_EXTENDED_PARAMETERS_dwFileAttributes.get(extendedParameters, 0L) as Int
			...
			CREATEFILE3_EXTENDED_PARAMETERS_dwFileAttributes.set(extendedParameters, 0L, attributes)
			CREATEFILE3_EXTENDED_PARAMETERS_dwFileFlags.set(extendedParameters, 0L, getFlags(features, data))
			...
			when (win32LastError) {
				...
				WindowsLastError.ERROR_FILE_NOT_FOUND.id.toInt() if expectDirectory -> {
					...
					if (status == 0) throwLastError()
					handle = nativeCreateFile3.invokeExact(
						capturedStateSegment,
						pathSegment,
						desiredAccess,
						shareMode,
