...
import org.bread_experts_group.ffi.windows.nativeGetTempPath2Wide
...
class WindowsSystemGetTemporaryStorageDeviceFeature : SystemGetPathDeviceFeature(
	...
	override fun supported(): Boolean = nativeGetTempPath2Wide != null
	override val device: SystemDevice
		get() {
			var size = nativeGetTempPath2Wide!!.invokeExact(capturedStateSegment, 0, MemorySegment.NULL) as Int
			if (size == 0) throwLastError()
			val filePathArena = Arena.ofShared()
			val filePathSegment = filePathArena.allocate(size * 2L)
			size = nativeGetTempPath2Wide.invokeExact(capturedStateSegment, size, filePathSegment) as Int
			if (size == 0) throwLastError()
