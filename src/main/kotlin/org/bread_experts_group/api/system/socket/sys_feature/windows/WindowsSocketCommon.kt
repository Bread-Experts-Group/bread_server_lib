...
import org.bread_experts_group.api.system.socket.sys_feature.windows.WindowsSocketEventManager.dropSocket
import org.bread_experts_group.ffi.capturedStateSegment
import org.bread_experts_group.ffi.windows.throwLastWSAError
import org.bread_experts_group.ffi.windows.wsa.WSAENOTCONN
...
fun winClose(
	...
	val closeData = mutableListOf<SocketCloseDataIdentifier>()
	...
	if (stopTx || stopRx) {
		val status = nativeShutdown!!.invokeExact(
			...
			socket,
			if (stopTx && stopRx) {
				closeData.add(StandardCloseFeatures.STOP_TX)
				closeData.add(StandardCloseFeatures.STOP_RX)
				2
			} else if (stopTx) {
				closeData.add(StandardCloseFeatures.STOP_TX)
				1
		...
		if (status != 0) when (wsaLastError) {
			WSAENOTCONN -> closeData.add(StandardSocketStatus.NOT_CONNECTED)
			else -> throwLastWSAError()
		}
	}
	if (features.contains(StandardCloseFeatures.RELEASE)) {
		closeData.add(StandardCloseFeatures.RELEASE)
