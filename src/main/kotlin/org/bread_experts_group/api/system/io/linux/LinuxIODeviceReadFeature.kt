...
import org.bread_experts_group.ffi.posix.nativeRead
import org.bread_experts_group.ffi.posix.throwLastErrno
import java.lang.foreign.MemorySegment
import java.util.concurrent.TimeUnit

class LinuxIODeviceReadFeature(private val handle: Int) : IODeviceReadFeature() {
	...
	override fun gatherSegments(
		...
		private fun read(): List<IOReceiveDataIdentifier> {
			...
			data.forEach { segment ->
				val readCount = nativeRead!!.invokeExact(
					capturedStateSegment,
					handle,
					segment,
					segment.byteSize()
				) as Long
				if (readCount == -1L) throwLastErrno()
				read += readCount
			}
