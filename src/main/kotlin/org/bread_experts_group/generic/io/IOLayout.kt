...
abstract class IOLayout<O> {
	companion object {
		val CHAR: PrimitiveIOLayout<Char> =
			PrimitiveIOLayout(
				{ r -> TODO("INTRANET") },
				{ w, i -> TODO("W") }
			)

		val UNSIGNED_SHORT: PrimitiveEndianIOLayout<UShort> =
			PrimitiveEndianIOLayout(
				{ r ->
		...
		val UNSIGNED_INT: PrimitiveEndianIOLayout<UInt> =
			PrimitiveEndianIOLayout(
				{ r ->
					this.name?.let { r.enter(it) }
					val v = r.u32()
		...
		): PrimitiveIOLayout<MappedEnumeration<T, E>> where E : Enum<E>, E : Mappable<E, T> =
			PrimitiveIOLayout(
