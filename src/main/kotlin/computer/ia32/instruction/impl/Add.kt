...
fun addAndSetFlagsCFOF8(processor: IA32Processor, a: UByte, b: UByte): UByte {
...
fun addAndSetFlagsCFOF16(processor: IA32Processor, a: UShort, b: UShort): UShort {
	addAuxCarryCheck(processor, a.toULong(), b.toULong())
...
fun addAndSetFlagsCFOF32(processor: IA32Processor, a: UInt, b: UInt): UInt {
	addAuxCarryCheck(processor, a.toULong(), b.toULong())
...
class Add : InstructionCluster {
	class TwoOperand8Bit(
		...
		override fun handle(processor: IA32Processor) {
			val (dest, src) = dc()
			val result = addAndSetFlagsCFOF8(processor, dest.first(), src.first())
			dest.second(result)
		...
		override fun handle(processor: IA32Processor) {
			when (processor.operandSize) {
				AddressingLength.R16 -> {
					...
					val result = addAndSetFlagsCFOF16(processor, dest16.first(), src16.first())
				...
				AddressingLength.R32 -> {
					...
					val result = addAndSetFlagsCFOF32(processor, dest32.first(), src32.first())
	...
	class TwoOperand8BitIncrement(
		...
		override fun handle(processor: IA32Processor) {
			...
			val dest = dc()
			val result = addAndSetFlagsCFOF8(processor, dest.first(), 1u)
			dest.second(result)
			setFlagsSFZFPF8(processor, result)
	...
	class TwoOperandIncrement(
		...
		override fun handle(processor: IA32Processor) {
			...
			when (processor.operandSize) {
				AddressingLength.R16 -> {
					val dest16 = dc16()
					val result = addAndSetFlagsCFOF16(processor, dest16.first(), 1u)
					dest16.second(result)
					setFlagsSFZFPF16(processor, result)
				}
				...
				AddressingLength.R32 -> {
					val dest32 = dc32()
					val result = addAndSetFlagsCFOF32(processor, dest32.first(), 1u)
					dest32.second(result)
					setFlagsSFZFPF32(processor, result)
				}
