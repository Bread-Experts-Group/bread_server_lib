...
import org.bread_experts_group.protocol.http.h2.setting.HTTP2SettingInitialWindowSize
import org.bread_experts_group.protocol.http.h2.setting.HTTP2SettingMaxConcurrentStreams
		...
		HTTPVersion.HTTP_0_9, HTTPVersion.HTTP_1_0, HTTPVersion.HTTP_1_1 -> {
			...
			fun decodeData(headers: Map<String, String>): ConsolidatedInputStream {
				...
				if (headers.contains("transfer-encoding")) {
					...
					data.streams.add(
						object : LockedLongInputStream(fromLock, 0uL) {
							var resetLength = true
							override var length: ULong = 0uL
								get() {
									if (resetLength) {
										resetLength = false
										field = asciiD.next(from, CRLF).toULong(16)
									}
									return field
								}
					...
					object : LockedLongInputStream(fromLock, headers.getValue("content-length").toULong()) {
